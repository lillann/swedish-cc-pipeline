...
class SimpleExtractor(PipelineStep):
    ...
    Omvandlar texten till bytes och avkodar med 'replace' för att
    förhindra dolda UnicodeDecodeError-krascher i BeautifulSoup.
    """

    def __init__(self):
        super().__init__()
        self.type = "🧼 Text Extractor"
        self.name = ""

    def run(self, data: DocumentsPipeline, rank: int = 0, world_size: int = 1) -> DocumentsPipeline:
        for doc in data:
            if simple_extractor(doc):
                yield doc


def simple_extractor(doc):
    try:
        soup = BeautifulSoup(doc.text, "lxml") 
    except (exceptions.ParserRejectedMarkup, AssertionError, Exception):
        # Om sidan är trasig kastar vi hela dokumentet
        return False

    doc.metadata["raw_html"] = doc.text

    # 1. Ta bort skräp på tagg- och klassnivå (Klasser först för prestanda!)
    skrap_selectors = [
        "[class*='cookie']",
        "[id*='cookie']",
        "[class*='menu']",
    ...
    # Vil tillåter kortare dokument om det är diskussionsforum
    min_length_allowed = 70 if doc_class == "discussion" else 150
