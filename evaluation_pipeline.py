...
from src.filters import DecodeUTF8Filter, OnlyHTMLFilter, SwedishQualityFilter, DropEmptyFilter
from src.filters import OnlyHTMLFilter
...
def run_experiment(pipeline_steps, experiment_name): 
    ...
    try:
        ...
        if not input_documents: return None
        ...
        valid_gold_documents = [
                   doc for doc in input_documents 
        ...
        if evaluator.diff_records:
            with open(filename, "w", encoding="utf-8") as f_diff:
                ...
                for rec in evaluator.diff_records:
                    ...
                    f_diff.write(f"❌ MISSLYCKADES AT EXTRAHERA (Fanns i guld, saknas i den extraherade texten):\n")
                    f_diff.write(f"   {', '.join(rec['missing']) if rec['missing'] else '[Inga missade ord]'}\n")
                    f_diff.write(f"➕ EXTRAHERADE FÖR MYCKET SKRÄP (Fanns inte i guld, kom med ändå):\n")
                    f_diff.write(f"   {', '.join(rec['extra']) if rec['extra'] else '[Inget extra skräp]'}\n")
                    f_diff.write("-" * 60 + "\n")
